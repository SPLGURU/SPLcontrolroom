    ...
    // --- ENVIRONMENT VARIABLES (Read from Netlify) ---
    // These should be set in your Netlify project's Environment Variables
    const SPL_API_BOOTSTRAP_STATIC = process.env.SPL_API_BOOTSTRAP_STATIC || 'https://en.fantasy.spl.com.sa/api/bootstrap-static/';
    const SPL_API_ENTRY_BASE = process.env.SPL_API_ENTRY_BASE || 'https://en.fantasy.spl.com.sa/api/entry/';
    const SPL_API_ELEMENT_SUMMARY_BASE = process.env.SPL_API_ELEMENT_SUMMARY_BASE || 'https://en.fantasy.spl.com.sa/api/element-summary/';
    ...
    async function getPlayerNameMap() {
        const url = SPL_API_BOOTSTRAP_STATIC; // Use environment variable
        try {
    ...
    async function getManagerBasicData(managerId) { 
        const managerEntryUrl = `${SPL_API_ENTRY_BASE}${managerId}/`; // Use environment variable
        try {
    ...
    async function getManagerHistoryAndCaptains(managerId, playerNameMap, managerBasicData) { 
        ...
        try {
            const transfersApiUrl = `${SPL_API_ENTRY_BASE}${managerId}/transfers/`; // Use environment variable
        ...
        for (let round = 1; round <= maxRounds; round++) {
            const picksUrl = `${SPL_API_ENTRY_BASE}${managerId}/event/${round}/picks`; // Use environment variable
        ...
        for (const { round, data } of sortedManagerPicksData) {
            ...
            if (transfersMadeInRound > 0) {
                ...
                for (const transfer of actualTransfersToProcess) {
                    ...
                    const playerInSummaryPromise = fetchWithRetry(`${SPL_API_ELEMENT_SUMMARY_BASE}${playerInId}/`); // Use environment variable
                    const playerOutSummaryPromise = fetchWithRetry(`${SPL_API_ELEMENT_SUMMARY_BASE}${playerOutId}/`); // Use environment variable
        ...
        for (const [captainIdStr, timesCaptained] of sortedCaptains) {
            ...
            const playerSummaryUrl = `${SPL_API_ELEMENT_SUMMARY_BASE}${captainId}/`; // Use environment variable
        ...
        const allPlayerSummaryPromises = Array.from(uniquePlayerIdsInSquad).map(async playerId => {
            try {
                const playerSummaryUrl = `${SPL_API_ELEMENT_SUMMARY_BASE}${playerId}/`; // Use environment variable
